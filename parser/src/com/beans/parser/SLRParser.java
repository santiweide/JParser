...
//                    System.out.println();
    ...
    public void doParse(ArrayList<String> tokens)
    {
        ...
        while(true)
        {
            ...
            //System.out.println("StateNum: " + ClosureSet.get(state.closure) + " token: " +token+ "\n"+ state);
            ...
            //System.out.println("Token " + token.toString().charAt(0));
            if(actionStr == null)
            {
                System.out.println("!! Error at '" + token.toString()+"'");
                while(i < tokens.size() && table[numC][tokens.get(i).toString().charAt(0)] == null)
                    i ++;
                if(i < tokens.size())
                {
